...
let handler = async (m, { conn, args, text, usedPrefix, command }) => {
  ...
  let who3 = m.mentionedJid[2]
      ...
      db.data.users[who3].money -= dapat * 1
                   ...
                   conn.sendMessage(m.chat, { text: `Mohon tunggu kak..\n\n@${name.split`@`[0]}\n@${who1.split`@`[0]}\n@${who2.split`@`[0]}\n@${who3.split`@`[0]}\nSedang berbisnis.. 😅\n\n*Kalian semua meletakkan modal masing-masing -${dapat} 😅*\n@${name.split`@`[0]}\n@${who1.split`@`[0]}\n@${who2.split`@`[0]}\n@${who3.split`@`[0]}`, mentions: m.mentionedJid }, { quoted: m })
                   setTimeout(() => {
                            conn.sendMessage(m.chat, { text: `Selamat statistik bisnis kalian meningkat\n\nMasing-masing mendapatkan:\n@${name.split`@`[0]} : *+${users[m.sender].money += untung * 1} Money*\n@${who1.split`@`[0]} : *+${users[who1].money += untung * 1} Money*\n@${who2.split`@`[0]} : *+${users[who2].money += untung * 1} Money*\n@${who3.split`@`[0]} : *+${users[who3].money += untung * 1} Money*`, mentions: m.mentionedJid }, { quoted: m })
                   }, 7200000)
                   setTimeout(() => {
                            conn.sendMessage(m.chat, { text: `Waahhh.. statistik bisnis kalian menurun\n\nMasing-masing minus:\n@${name.split`@`[0]} : *-${users[m.sender].money -= rugi * 1} Money*\n@${who1.split`@`[0]} : *-${users[who1].money -= rugi * 1} Money*\n@${who2.split`@`[0]} : *-${users[who2].money -= rugi * 1} Money*\n@${who3.split`@`[0]} : *-${users[who3].money -= rugi * 1} Money*`, mentions: m.mentionedJid }, { quoted: m })
                   }, 14400000) 
                   setTimeout(() => {
                            conn.sendMessage(m.chat, { text: `Selamat statistik bisnis kalian meningkat\n\nMasing-masing mendapatkan:\n@${name.split`@`[0]} : *+${users[m.sender].money += untung * 1} Money*\n@${who1.split`@`[0]} : *+${users[who1].money += untung * 1} Money*\n@${who2.split`@`[0]} : *+${users[who2].money += untung * 1} Money*\n@${who3.split`@`[0]} : *+${users[who3].money += untung * 1} Money*`, mentions: m.mentionedJid }, { quoted: m })
                   }, 28800000) 
